window.expanded_nodes = window.expanded_nodes || new Set();
        ...
        <div class="hierarchy-row"
            ...
            <div class="h-summary">
                ...
                <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;margin-left:8px;">
                    ...
                    <div style="display:flex;flex-direction:column;align-items:center;line-height:1;">
                        <span style="font-size:8px;opacity:0.7;margin-bottom:2px;letter-spacing:0.3px;">WEIGHTAGE</span>
    ...
    function attach_events(frm, all_tasks) {
        ...
        wrapper.find(".update-item").off("click").on("click", function (e) {
            ...
            frappe.db.get_value("Task", task_name, ["project", "subject", "parent_task", "custom_is_level_task"]).then(r => {
                let project      = r.message && r.message.project;
                let task_subject = r.message && r.message.subject;
                let is_level_task = r.message && r.message.custom_is_level_task;
                ...
                if (is_level_task) {
                    options.push({ key: "level_sheet", label: "Task Level Sheet", doctype: "Task Level Sheet" });
                }
                ...
                d.$wrapper.find(".update-option-card").on("dblclick", function () {
                    ...
                    else if (sel_key === "level_sheet")        { open_task_level_sheet_dialog(task_name, project); }
    ...
    function open_task_level_sheet_dialog(task_name, project) {
        let today = frappe.datetime.get_today();
        ...
        function map_hierarchy(ancestors) {
            const level_fieldnames  = ["stage", "task", "task_level1", "task_level2", "task_level3",
                "task_level4", "task_level5", "task_level6", "task_level7", "task_level8", "task_level9", "task_level10"];
       
            const subject_fieldnames = ["stage_subject", "task_subject", "task_level1_subject", "task_level2_subject",
                "task_level3_subject", "task_level4_subject", "task_level5_subject", "task_level6_subject",
                "task_level7_subject", "task_level8_subject", "task_level9_subject_copy", "task_level10_subject"];

            let values = {};
            ancestors.forEach((a, i) => {
                if (i >= level_fieldnames.length) return; 
                values[level_fieldnames[i]]   = a.name;
                values[subject_fieldnames[i]] = a.subject;
            });
            return values;
        }
        ...
        Promise.all([
            frappe.db.get_value("Project", project || "", "custom_site"),
            get_ancestors(task_name)
        ]).then(([proj_r, ancestors]) => {
            let site = proj_r.message && proj_r.message.custom_site;
            let hierarchy_values = map_hierarchy(ancestors);

            function calculate_average() {
                let rows = (d.fields_dict.level_sheet_details.df.data || []).filter(row => row.__islocal !== undefined || true);
                let rl_values = rows
                    .filter(row => row.rl !== undefined && row.rl !== null && row.rl !== "")
                    .map(row => flt(row.rl));
                let avg = rl_values.length ? (rl_values.reduce((a, b) => a + b, 0) / rl_values.length) : 0;
            ...
            dialog_fields.push(
                { fieldtype: "Section Break", label: __("Level Sheet Details") },
                {
                    ...
                    options: "Task Level Sheet Details", data: [{}],
                    fields: [
                        { label: __("Design"),  fieldname: "design", fieldtype: "Float", in_list_view: 1, columns: 2 },
                        { label: __("B.S."),    fieldname: "bs",     fieldtype: "Float", in_list_view: 1, columns: 2 },
                        { label: __("I.S"),     fieldname: "is",     fieldtype: "Float", in_list_view: 1, columns: 2 },
                        { label: __("F.S."),    fieldname: "fs",     fieldtype: "Float", in_list_view: 1, columns: 2 },
                        { label: __("H.I"),     fieldname: "hi",     fieldtype: "Float", in_list_view: 1, columns: 2 },
                        { label: __("R.L."),    fieldname: "rl",     fieldtype: "Float", in_list_view: 1, columns: 2,
                            onchange: function () { calculate_average(); } },
                        { label: __("Remark"),  fieldname: "remark", fieldtype: "Data",  in_list_view: 1, columns: 2 }
                    ]
                },
            ...
            var d = new frappe.ui.Dialog({
                ...
                primary_action_label: __("Save"),
                primary_action(values) {
                    if (!values.level_sheet_details || !values.level_sheet_details.length) {
                        frappe.msgprint(__("Please add at least one Level Sheet Details row"));
                        return;
                    }

                    let rl_values = values.level_sheet_details
                        .filter(row => row.rl !== undefined && row.rl !== null && row.rl !== "")
                        .map(row => flt(row.rl));
                    let final_average = rl_values.length ? flt((rl_values.reduce((a, b) => a + b, 0) / rl_values.length).toFixed(3)) : 0;
                    ...
                    frappe.call({
                        ...
                        args: { doc: {
                            ...
                            level_sheet_details: (values.level_sheet_details || []).map(row => ({
                                ...
                                design: row.design, bs: row.bs, is: row.is, fs: row.fs, hi: row.hi, rl: row.rl, remark: row.remark
                            }))
                        }},
                        freeze: true, freeze_message: __("Saving Task Level Sheet..."),
                        callback(r) {
                            if (r.message) {
                                frappe.msgprint({ title: __("Task Level Sheet Created"),
                                    message: __("Task Level Sheet <a href='/app/task-level-sheet/{0}'><b>{0}</b></a> saved.", [r.message.name]),
                                    indicator: "green" });
                                d.hide();
                            }
                        }

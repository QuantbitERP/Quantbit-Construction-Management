...
@frappe.whitelist()
def get_portfolio_dashboard_data(from_date=None, to_date=None, project=None, site=None):
    # Base conditions for Project
    proj_conditions = ["is_active = 'Yes'"]
    proj_values = {}
    ...
    # Fetch filter options for UI dropdowns
    projects_list = frappe.db.sql("""
        SELECT name, IFNULL(project_name, name) as project_name 
        FROM `tabProject` 
        WHERE is_active = 'Yes'
    ...
    # Fetch projects for the matrix table
    matrix_projects_raw = frappe.db.sql(f"""
        SELECT 
            p.name as id,
            IFNULL(p.project_name, p.name) as name,
    ...
    for idx, p in enumerate(matrix_projects_raw):
        ...
        matrix_projects.append({
            ...
            "ac": ac_amt,
            "site": p.get("site"),
            "contract_value_raw": c_val
        })

    # Prepare response based on filter type
    if project and project != "all":
        # Scenario 1: Only return data for this project
        project_data = matrix_projects[0] if matrix_projects else None
        if project_data:
            project_data.pop("contract_value_raw", None)
        return {
            "total_contract_value": total_contract_value,
            "billed_to_date": billed_to_date,
            "active_projects_count": active_projects_count,
    ...
    if site and site != "all":
        # Scenario 2: Return only projects of that site
        for p in matrix_projects:
            p.pop("contract_value_raw", None)
        return {
            "total_contract_value": total_contract_value,
            "billed_to_date": billed_to_date,
            "active_projects_count": active_projects_count,
    ...
    # Scenario 3: Neither project nor site given. Return all sites and projects hierarchically.
    sites_dict = {}
    unassigned_projects = []

    for p in matrix_projects:
        p_site = p.get("site")
        c_val = p.pop("contract_value_raw", 0.0)
        b_amt = p.get("billed_amount", 0.0)

        if not p_site:
            unassigned_projects.append(p)
            continue
        
        if p_site not in sites_dict:
            sites_dict[p_site] = {
                "site": p_site,
        ...
        sites_dict[p_site]["projects"].append(p)
        sites_dict[p_site]["total_contract_value"] += c_val
        sites_dict[p_site]["billed_to_date"] += b_amt
        sites_dict[p_site]["active_projects_count"] += 1
